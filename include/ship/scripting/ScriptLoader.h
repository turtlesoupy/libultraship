...
class ScriptLoader {
  public:
     ...
     *                    ownership correctly.
     * @param postInitCb Optional callback invoked after ModInit returns,
     *                    used by the host to clear the current owner.
     */
    void LoadAll(
        const std::optional<std::function<void(const std::string&)>>& preInitCb = std::nullopt,
        const std::optional<std::function<void(const std::string&)>>& postInitCb = std::nullopt);
     ...
     */
    void UnloadAll(
        const std::optional<std::function<void(const std::string&)>>& preExitCb = std::nullopt,
        const std::optional<std::function<void(const std::string&)>>& postExitCb = std::nullopt);
  ...
  private:
    ...
    SafeLevel mSafeLevel = SafeLevel::ALLOW_ALL_SCRIPTS;
