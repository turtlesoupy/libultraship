...
void FixELFHeader(const std::string& path) {
    ...
    // 2. ELF Surgery — TCC emits a NOTE/EH_FRAME segment that the kernel
    // mmap path on some glibc versions interprets as requiring W^X. Flip
    // it to PT_GNU_STACK with RW (no-execute) so dlopen accepts it.
